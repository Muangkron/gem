...
with st.sidebar:
    ...
    st.markdown("**Model 1 (เกลียว 4/6/10 - ฐานมุม 155°):**")
    st.latex(r"x = |\theta - 155|")
    st.latex(r"\text{Brix} = -0.0196x^2 + 0.0045x + 16.757")

    st.markdown("**Model 2 (เกลียว 7/11/17 - ฐานมุม 136°):**")
        ...
        if not api_key:
            ...
            with st.spinner("Gemini AI กำลังวิเคราะห์แนวเกลียวตาสับปะรด..."):
                try:
                    ...
                    # Prompt บังคับเลือกโมเดลตามเกณฑ์ที่ถูกต้อง
                    prompt = """
                    ...
                    1. **การหาเส้นอ้างอิงและการวัดมุม:**
                       ...
                       - วัดมุมแหลม ϕ ที่เส้นเกลียวทำกับเส้นแนวนอนฝั่งขวา
                       - คำนวณมุมจริง θ = 180° - ϕ (เช่น 180° - 41° = 139°)

                    2. **การจำแนกรูปแบบเกลียวและเลือกโมเดล (เน้นย้ำความถูกต้อง):**
                       - ตรวจประเมินรูปแบบเกลียวตาตามลำดับ:
                         * หากเป็นกลุ่มเกลียว 7/11/17 (ทรงยาว / แนวเกลียวใกล้เคียง 136°) -> ให้เลือก "Model 2"
                         * หากเป็นกลุ่มเกลียว 4/6/10 (ทรงกลมป้อม / แนวเกลียวใกล้เคียง 155°) -> ให้เลือก "Model 1"
                    ...
                    ตอบกลับเฉพาะข้อความ JSON รูปแบบนี้เท่านั้น (ห้ามมีคำเกริ่นหรือ Markdown Block):
                    {
                      ...
                      "reasoning": "อธิบายสั้นๆ ว่าพบมุมแหลมกี่องศา คำนวณมุม θ ได้เท่าใด และเลือกโมเดลอย่างไร"
                    }
                    """
                    ...
                    if json_match:
                        ...
                        selected_model = data.get("selected_model", "Model 2")
